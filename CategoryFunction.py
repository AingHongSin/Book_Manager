...
class NewCategoriesAction():
    def __init__(self):

        self.VarName = tk.StringVar()
        self.CategoriesName = []

        self.addCategories_Interface = tk.Toplevel()
        self.addCategories_Interface.title("Category Adding")

        self.mainFrame_CategoriesAdding_topLevel = tk.Frame(self.addCategories_Interface)
        self.mainFrame_CategoriesAdding_topLevel.pack()

        self.buttonFrame_CategoriesAdding_topLevel = tk.Frame(self.addCategories_Interface)
        self.buttonFrame_CategoriesAdding_topLevel.pack()

        self.lblNameCategories = tk.Label(self.mainFrame_CategoriesAdding_topLevel, text = 'Name')
        self.lblNameCategories.grid(row = 0, column = 0)

        self.inputNameCategories = tk.Entry(self.mainFrame_CategoriesAdding_topLevel, width = '20', textvariable = self.VarName)
        self.inputNameCategories.grid(row = 0, column = 1)
        self.inputNameCategories.focus()

        self.btnCancel = tk.Button(self.buttonFrame_CategoriesAdding_topLevel, text = 'Cancel', width = '10', command = self.addCategories_Interface.destroy)
        self.btnCancel.grid(row = 0, column = 0)

        self.btnDone = tk.Button(self.buttonFrame_CategoriesAdding_topLevel, text = 'Done', width = '10', command = self.AddCategoriesName_to_Database)
    ...
    def AddCategoriesName_to_Database(self):
        ...
        CategoriesName_in_CategoriesTable = self.VarName.get()

        if  CategoriesName_in_CategoriesTable != '':
            Database.addCategories(CategoriesName_in_CategoriesTable)
            self.inputNameCategories.delete(0, 'end')
            self.addCategories_Interface.destroy()

            self.CategoriesName.append(CategoriesName_in_CategoriesTable)

            self.c.execute("INSERT INTO Categories VALUES (?)", self.CategoriesName[0:])
            self.conn.commit()
